...
function fixedScore(category: Category, race: RaceId, fallback: number): number {
  return raceDef(race).scoreOverride[category] ?? fallback
}

function withRaceBonus(base: number, category: Category, race: RaceId): number {
  if (base <= 0 && category !== 'bagOfHolding') return base
  const bonus = raceDef(race).categoryBonus[category] ?? 0
  return base + bonus
}

export function categoryMaxScore(category: Category, race: RaceId = 'human'): number {
  ...
  switch (category) {
    ...
    case 'bagOfHolding':
      return BAG_OF_HOLDING_MAX + (def.categoryBonus.bagOfHolding ?? 0)
    case 'balancedParty':
      return BALANCED_PARTY_SCORE
    case 'initiative':
      return INITIATIVE_SCORE + (def.categoryBonus.initiative ?? 0)
    case 'dungeonCrawl':
      return DUNGEON_CRAWL_SCORE + (def.categoryBonus.dungeonCrawl ?? 0)
    case 'polymorph':
      return POLYMORPH_SCORE + (def.categoryBonus.polymorph ?? 0)
    case 'minMax':
      return MIN_MAX_SCORE
    case 'success':
      return SUCCESS_SCORE
    case 'criticalSuccess':
      return fixedScore('criticalSuccess', race, CRITICAL_SUCCESS_SCORE)
    case 'failure':
      return FAILURE_SCORE
    case 'criticalFailure':
      return fixedScore('criticalFailure', race, CRITICAL_FAILURE_SCORE)
    case 'twinnedSpell':
      return TWINNED_SPELL_SCORE + (def.categoryBonus.twinnedSpell ?? 0)
  }
}

export function rawCategoryScore(
  dice: number[],
  category: Category,
  race: RaceId = 'human',
): number {
  ...
  switch (category) {
    case 'magicMissile':
      base = maxKind >= 3 ? sum : 0
      break
    case 'partyOfFour':
      base = maxKind >= 4 ? sum : 0
      break
    case 'fireball':
      base = maxKind >= 5 ? sum : 0
      break
    case 'balancedParty':
      base = isFullHouse(counts) ? BALANCED_PARTY_SCORE : 0
      break
    case 'initiative':
      base = hasSequence(uniqueSortedFaces(dice), 5) ? INITIATIVE_SCORE : 0
      break
    case 'dungeonCrawl':
      base = hasSequence(uniqueSortedFaces(dice), 8) ? DUNGEON_CRAWL_SCORE : 0
      break
    case 'polymorph':
      base = isPolymorph(dice) ? POLYMORPH_SCORE : 0
      break
    case 'minMax':
      base = isMinMax(dice) ? MIN_MAX_SCORE : 0
      break
    case 'success':
      base = isSuccess(dice) ? SUCCESS_SCORE : 0
      break
    case 'criticalSuccess':
      base = isCriticalSuccess(dice)
        ? fixedScore('criticalSuccess', race, CRITICAL_SUCCESS_SCORE)
        : 0
      break
    case 'failure':
      base = isFailure(dice) ? FAILURE_SCORE : 0
      break
    case 'criticalFailure':
      base = isCriticalFailure(dice)
        ? fixedScore('criticalFailure', race, CRITICAL_FAILURE_SCORE)
        : 0
      break
    case 'twinnedSpell':
      base = isTwinnedSpell(dice) ? TWINNED_SPELL_SCORE : 0
      break
    case 'bagOfHolding':
      base = sum
      break
  ...
  return withRaceBonus(base, category, race)
}
...
export function scoreForCategory(state: GameState, category: Category): number {
  ...
  return rawCategoryScore(state.dice, category, state.race)
}
...
export function inspirationThresholdFor(race: RaceId): number {
  return raceDef(race).inspirationThreshold
}

export function inspirationBonus(scorecard: Scorecard, race: RaceId = 'human'): number {
  const threshold = inspirationThresholdFor(race)
  return upperTotal(scorecard) >= threshold ? INSPIRATION_POINTS : 0
}
...
export function totalScore(state: GameState): number {
  ...
  t += inspirationBonus(state.scorecard, state.race)
  return t
}
...
export function applyScoreMut(state: GameState, category: Category): void {
  ...
  state.rollsRemaining = ROLLS_PER_TURN
  for (let i = 0; i < state.held.length; i++) state.held[i] = false
  state.hasRolled = false
  state.luckyRerollsLeft = raceDef(state.race).luckyRerollsPerTurn
}
